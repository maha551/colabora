...
## Minimum Safeguards (Organization-Configurable)

These rules set organization-configurable floors. Organizations may change them via rule proposals and voting. The platform still applies a dynamic quorum backstop based on organization size (see `safety-mechanisms.js`).

### 1. Minimum Quorum
- **Rule:** `minimum_quorum_percentage` (default: 0.1 = 10%)
- **Purpose:** Ensure some participation before decisions are made
- **Enforcement:** All votes must meet the configured minimum quorum (and dynamic platform floor)
- **Configurable:** Yes, via democratic rule proposals

### 2. Minimum Approval Threshold
- **Rule:** `minimum_approval_threshold` (default: 0.5 = 50%)
- **Purpose:** Prevent minority rule
- **Enforcement:** Intended floor for approvals (org-configured)
- **Configurable:** Yes, via democratic rule proposals

### 3. Minimum Voting Period
- **Rule:** `minimum_voting_period_hours` (default: 24 hours)
- **Purpose:** Give members time to participate
- **Enforcement:** Voting cannot end before the configured minimum period
- **Configurable:** Yes, via democratic rule proposals
