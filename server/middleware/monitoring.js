...
class MetricsCollector {
  ...
  recordError(errorType, details = {}) {
    ...
    if (!metrics.errors.byType.has(errorType)) {
      metrics.errors.byType.set(errorType, { count: 0, lastOccurrence: null });
    }
    const errorStats = metrics.errors.byType.get(errorType);
    errorStats.count++;
    errorStats.lastOccurrence = new Date();
    
    logger.warn(`Error recorded: ${errorType}`, details);
  }
  ...
  getMetricsSummary() {
    ...
    return {
      ...
      errors: {
        ...
        byType: Array.from(metrics.errors.byType.entries())
