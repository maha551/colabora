...
interface TreeProposalDecisionCardProps {
  entry: DecisionEntry;
...
export function TreeProposalDecisionCard({
  entry,
  onNavigateToDocument,
  organizationBorderColor,
  hideContextLinks = false,
}: TreeProposalDecisionCardProps) {
  const { t } = useTranslation('activity');
  const { cardActions } = useDesignSystemLabels();
  const { formatDate, formatTime } = useTimezone();
  const p = entry.payload as Record<string, unknown>;
  const operationType = p.operationType ? String(p.operationType) : t('item.change');
  ...
  return (
    <DecisionCardShell
      icon="Folder"
      title={t('item.documentTree', { operationType })}
      meta={meta}
      voteBar={voteBar}
