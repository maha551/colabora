...
// Database migration endpoint (admin only, temporary)
router.post('/run-migration', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    console.log('Setting Diana Prince as admin...');
    await runDbQuery(db, `
    ...
    for (const tableSql of governanceTables) {
      await runDbQuery(db, tableSql);
    }

    console.log('✅ Governance tables created');

    // Log migration completion
    logAudit(db, null, 'database_migration', adminId, null, {
      migration: 'governance_tables_added',
      tablesCreated: governanceTables.length
    }, req);
...
function runDbQuery(db, sql, params = []) {
  return new Promise((resolve, reject) => {
