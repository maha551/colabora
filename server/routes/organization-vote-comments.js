/**
 * Comments for organization votes (organization-scoped).
 * Routes: GET/POST/PUT/DELETE under /api/organizations/:organizationId/votes/:voteId/comments
 */
...
const { validateOrganizationVoteForComment, formatCommentResponse } = require('../utils/commentHelpers');

const router = express.Router({ mergeParams: true });

const COMMENTABLE_TYPE = 'organization_vote';

function entityId(req) {
  return req.params.voteId;
}

// GET
router.get('/', requireAuth, requireOrganizationMember, asyncHandler(async (req, res) => {
  const db = req.app.locals.db;
  const voteId = entityId(req);
  const organizationId = req.params.organizationId;

  const exists = await validateOrganizationVoteForComment(db, voteId, organizationId);
  if (!exists) {
    throw new ApiError(404, 'Organization vote not found', 'NOT_FOUND');
  }
  ...
  const query = `
    ...
    LIMIT ? OFFSET ?
  `;
  const comments = await TransactionManager.queryAll(db, query, [COMMENTABLE_TYPE, voteId, limit, offset]);
...
router.post('/', requireAuth, requireOrganizationMember, ...commentValidation.create, asyncHandler(async (req, res) => {
  ...
  if (parentId) {
    ...
    `, [parentId, COMMENTABLE_TYPE, voteId]);
    if (!parentComment) {
      throw new ApiError(400, 'Parent comment not found or does not belong to this vote', 'VALIDATION_ERROR');
    }
  ...
  await TransactionManager.execute(db, `
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `, [commentId, COMMENTABLE_TYPE, voteId, userId, text.trim(), parentId || null]);
  ...
  webSocketManager.broadcastOrganizationUpdate(organizationId, 'organization-vote-comment', {
    type: 'organization-vote-comment',
    voteId,
    comment: result,
    action: 'created'
  });
...
router.put('/:commentId', requireAuth, requireOrganizationMember, ...commentValidation.update, asyncHandler(async (req, res) => {
  ...
  `, [commentId, COMMENTABLE_TYPE, voteId]);
  if (!comment) {
