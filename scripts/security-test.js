...
// Scan request-handling code only (database/bootstrap may contain demo fixtures).
const scanDirs = ['server/routes', 'server/middleware', 'server/modules', 'server/services'];
    ...
    if (stat.isDirectory() && !['node_modules', '.git', 'build'].includes(item)) {
      ...
      try {
        ...
        const patterns = [
          ...
          /(?:secret|api[_-]?key)\s*[:=]\s*['"][^'"]{8,}['"]/i,
          /(?:access[_-]?token|auth[_-]?token)\s*[:=]\s*['"][^'"]{10,}['"]/i,
        ];
...
try {
  ...
  const serverDepsInClient = clientDeps.filter(dep =>
    ['bcryptjs', 'cors', 'express', 'helmet', 'jsonwebtoken'].includes(dep)
  );
...
console.log('   - Regularly rotate JWT_SECRET');
