/**
 * Comments for document tree proposals (document-scoped).
 * Mount at: /api/documents/:documentId/document-tree-proposals/:proposalId/comments
...
const { validateTreeProposalForComment, formatCommentResponse } = require('../utils/commentHelpers');

const router = express.Router({ mergeParams: true });

const COMMENTABLE_TYPE = 'tree_proposal';
...
router.get('/', requireAuth, requireDocumentAccess, asyncHandler(async (req, res) => {
  ...
  const exists = await validateTreeProposalForComment(db, proposalId, documentId);
  if (!exists) {
    throw new ApiError(404, 'Tree proposal not found', 'NOT_FOUND');
  }
...
router.post('/', requireAuth, requireDocumentAccess, ...commentValidation.create, asyncHandler(async (req, res) => {
  ...
  if (doc?.organization_id) {
    webSocketManager.broadcastOrganizationUpdate(doc.organization_id, 'tree-proposal-comment', {
      type: 'tree-proposal-comment',
      documentId,
      proposalId,
      comment: result,
      action: 'created'
  ...
  webSocketManager.broadcastCommentUpdate(documentId, proposalId, null, result, 'created', COMMENTABLE_TYPE);
