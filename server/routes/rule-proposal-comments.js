/**
 * Comments for governance rule proposals (organization-scoped).
 * Routes: GET/POST/PUT/DELETE under /api/organizations/:organizationId/rule-proposals/:proposalId/comments
 */
...
const { validateRuleProposalForComment, formatCommentResponse } = require('../utils/commentHelpers');

const router = express.Router({ mergeParams: true });

const COMMENTABLE_TYPE = 'rule_proposal';

// GET /api/organizations/:organizationId/rule-proposals/:proposalId/comments
router.get('/', requireAuth, requireOrganizationMember, asyncHandler(async (req, res) => {
  const db = req.app.locals.db;
  const proposalId = req.params.proposalId;
  const organizationId = req.params.organizationId;

  const exists = await validateRuleProposalForComment(db, proposalId, organizationId);
  if (!exists) {
    throw new ApiError(404, 'Rule proposal not found', 'NOT_FOUND');
  }
  ...
  const query = `
    ...
    LIMIT ? OFFSET ?
  `;
  const comments = await TransactionManager.queryAll(db, query, [COMMENTABLE_TYPE, proposalId, limit, offset]);
...
// POST /api/organizations/:organizationId/rule-proposals/:proposalId/comments
router.post('/', requireAuth, requireOrganizationMember, ...commentValidation.create, asyncHandler(async (req, res) => {
  ...
  if (parentId) {
    ...
    `, [parentId, COMMENTABLE_TYPE, proposalId]);
    if (!parentComment) {
      throw new ApiError(400, 'Parent comment not found or does not belong to this proposal', 'VALIDATION_ERROR');
    }
  ...
  await TransactionManager.execute(db, `
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `, [commentId, COMMENTABLE_TYPE, proposalId, userId, text.trim(), parentId || null]);

  let comment = await TransactionManager.query(db, `
    SELECT c.*,
           u.name as user_name,
           u.email as user_email,
           u.avatar as user_avatar,
  ...
  webSocketManager.broadcastOrganizationUpdate(organizationId, 'rule-proposal-comment', {
    type: 'rule-proposal-comment',
    proposalId,
    comment: result,
    action: 'created'
  });
...
// PUT /api/organizations/:organizationId/rule-proposals/:proposalId/comments/:commentId
router.put('/:commentId', requireAuth, requireOrganizationMember, ...commentValidation.update, asyncHandler(async (req, res) => {
  ...
  `, [commentId, COMMENTABLE_TYPE, proposalId]);
  if (!comment) {
...
// DELETE /api/organizations/:organizationId/rule-proposals/:proposalId/comments/:commentId
