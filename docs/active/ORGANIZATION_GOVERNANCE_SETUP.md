...
1. **System Admin** (highest level)
   - Can create organizations (Admin Dashboard only)
   - Can deactivate organizations (`is_active = false`) or permanently delete inactive orgs (admin API)
   - Can access any organization via admin bypass (without membership)
   - Can add/remove members and manage representatives via admin org panel or org routes (when admin)
   - Can suspend users and promote/demote admins
...
**Vote Changes and Finalization Timing:**
- When `voteChangeAllowed` is **true** (flexible voting), document-level voting stays open until `voting_deadline` even if quorum and the acceptance threshold are already met. Members can continue to cast or change votes until the deadline; the document is finalized at the deadline (scheduler) or when a representative manually completes voting after the deadline.
- When `voteChangeAllowed` is **false** (locked voting), the document may transition to `agreed` automatically as soon as quorum and the acceptance threshold are met — without waiting for the voting deadline.
- Manual early completion (Complete vote) before the deadline is only available when vote changes are **not** allowed. If vote changes are allowed, representatives must wait until the voting deadline passes before manually finalizing.
